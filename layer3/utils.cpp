...
namespace layer_3 {
    ...
    )
    {
        ...
        for(int i = 0; i < OUT_BUF_HEIGHT; i++)
        {
            ...
            for(int j = 0; j < OUT_BUF_WIDTH; j++)
            {
                ...
                out_fm[f][height_offset + i][width_offset + j] = out_fm_buf[i][j] + (fm_t) bias_buf[f];
            }
        }
    }
    ...
    void resnet_add_residual_fm(
        fm_t feature_map[OUT_FM_DEPTH][OUT_FM_HEIGHT][OUT_FM_WIDTH],
        fm_t residual_map[OUT_FM_DEPTH][OUT_FM_HEIGHT][OUT_FM_WIDTH],
        bool enable_relu)
    {
    for(int k = 0; k < OUT_FM_DEPTH; k++)
    {
